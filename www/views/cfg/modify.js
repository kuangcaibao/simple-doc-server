...
var getCfg = function(id) {
  $.post("/api/getCfgDetail", {
    ...
    if(data.errorCode == 0) {
      vm.selectLabels = data.cfg.labels.split(",");
      vm.title = data.cfg.title;
      vm.abstract = data.cfg.abstract;
      vm.effective = data.cfg.effective;
      mde.value(data.cfg.content);
    } else {
      alert(data.errorInfo);
    }
  })
